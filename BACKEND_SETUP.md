...
3. `chatbotLead` で無料診断申込者への自動返信メールを実装
4. `contactInquiry` で問い合わせ保存と管理者通知メールを実装
5. `adminLogin` をサーバー側認証に切り替え
6. `adminLeads` で管理画面から受付データを確認
7. Stripeの本番決済リンク、またはCheckout作成APIを設定
8. `app-config.js` に本番API URLを設定
9. 送信テスト、通知テスト、ファイル確認、決済テストを実施
...
`RESEND_API_KEY` と `FROM_EMAIL` を設定すると、無料診断受付後に申込者へ受付完了メール、管理者へ通知メールが送信されます。未設定の場合も、受付データとファイル保存は止まらず、メール送信だけスキップします。
